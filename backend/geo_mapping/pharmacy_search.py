"""
pharmacy_search.py ΓÇö Pharmacy search algorithm for MediFind.
...
No RPC function needed ΓÇö queries the REST API directly.
"""
...
# ΓöÇΓöÇ Supabase config (read at call-time for robustness) ΓöÇΓöÇ
...
async def search_pharmacies_by_names(
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        ...
        # ΓöÇΓöÇ Step 1: Fetch all pharmacies (including external DB info) ΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ Step 2: Filter by distance ΓöÇΓöÇ
        nearby = []
        ...
        # ΓöÇΓöÇ Step 3: Group pharmacies by their inventory source ΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ Step 4: Fetch inventory from all sources in parallel ΓöÇΓöÇ
        tasks = [
    ...
    # ΓöÇΓöÇ Step 4: Build per-pharmacy inventory index ΓöÇΓöÇ
    ...
    # ΓöÇΓöÇ Step 5: Match medicines per pharmacy ΓöÇΓöÇ
    ...
    # ΓöÇΓöÇ Step 6: Sort and categorize ΓöÇΓöÇ
...
def _parse_quantity(raw: str | int) -> int:
    """Parse quantity from OCR output ΓÇö may be '10', '10 tablets', etc."""
...
def _build_suggestion(partials: list[PharmacyResult], total: int) -> str:
    ...
    if missing == 1:
        return (
            ...
            f"1 medicine is unavailable ΓÇö try an alternative brand or a second pharmacy."
        )
